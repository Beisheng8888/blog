<template><div><h1 id="分布式搜索引擎03" tabindex="-1"><a class="header-anchor" href="#分布式搜索引擎03" aria-hidden="true">#</a> 分布式搜索引擎03</h1>
<h1 id="_1-数据聚合" tabindex="-1"><a class="header-anchor" href="#_1-数据聚合" aria-hidden="true">#</a> 1.数据聚合</h1>
<p>**<a href="https://www.elastic.co/guide/en/elasticsearch/reference/current/search-aggregations.html" target="_blank" rel="noopener noreferrer">聚合（<ExternalLinkIcon/></a><a href="https://www.elastic.co/guide/en/elasticsearch/reference/current/search-aggregations.html" target="_blank" rel="noopener noreferrer">aggregations<ExternalLinkIcon/></a><a href="https://www.elastic.co/guide/en/elasticsearch/reference/current/search-aggregations.html" target="_blank" rel="noopener noreferrer">）<ExternalLinkIcon/></a>**可以让我们极其方便的实现对数据的统计、分析、运算。例如：</p>
<ul>
<li>什么品牌的手机最受欢迎？</li>
...
<h2 id="_1-2-dsl实现聚合" tabindex="-1"><a class="header-anchor" href="#_1-2-dsl实现聚合" aria-hidden="true">#</a> 1.2.DSL实现聚合</h2>
<p>现在，我们要统计所有数据中的酒店品牌有几种，其实就是按照品牌对数据分组。此时可以根据酒店品牌的名称做聚合，也就是Bucket聚合。</p>
<h3 id="_1-2-1-bucket聚合语法" tabindex="-1"><a class="header-anchor" href="#_1-2-1-bucket聚合语法" aria-hidden="true">#</a> 1.2.1.Bucket聚合语法</h3>
<p>语法如下：</p>
<div class="language-json line-numbers-mode" data-ext="json"><pre v-pre class="language-json"><code>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><div class="language-json line-numbers-mode" data-ext="json"><pre v-pre class="language-json"><code>GET /hotel/_search
...
<figure><img src="@source/src/必会工具/assets/image-20210723171948228.png" alt="image-20210723171948228" tabindex="0" loading="lazy"><figcaption>image-20210723171948228</figcaption></figure>
...
<figure><img src="@source/src/必会工具/assets/image-20210723172404836.png" alt="image-20210723172404836" tabindex="0" loading="lazy"><figcaption>image-20210723172404836</figcaption></figure>
<h3 id="_1-2-4-metric聚合语法" tabindex="-1"><a class="header-anchor" href="#_1-2-4-metric聚合语法" aria-hidden="true">#</a> 1.2.4.Metric聚合语法</h3>
<p>beisheng，我们对酒店按照品牌分组，形成了一个个桶。现在我们需要对桶内的酒店做运算，获取每个品牌的用户评分的min、max、avg等值。</p>
...
<span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这次的score_stats聚合是在brandAgg的聚合内部嵌套的子聚合。因为我们需要在每个桶分别计算。</p>
<p>另外，我们还可以给聚合结果做个排序，例如按照每个桶的酒店平均分做排序：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723172917636.png" alt="image-20210723172917636" tabindex="0" loading="lazy"><figcaption>image-20210723172917636</figcaption></figure>
...
<h2 id="_1-3-restapi实现聚合" tabindex="-1"><a class="header-anchor" href="#_1-3-restapi实现聚合" aria-hidden="true">#</a> 1.3.RestAPI实现聚合</h2>
<h3 id="_1-3-1-api语法" tabindex="-1"><a class="header-anchor" href="#_1-3-1-api语法" aria-hidden="true">#</a> 1.3.1.API语法</h3>
<p>聚合条件与query条件同级别，因此需要使用request.source()来指定聚合条件。</p>
<p>聚合条件的语法：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723173057733.png" alt="image-20210723173057733" tabindex="0" loading="lazy"><figcaption>image-20210723173057733</figcaption></figure>
<p>聚合的结果也与查询结果不同，API也比较特殊。不过同样是JSON逐层解析：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723173215728.png" alt="image-20210723173215728" tabindex="0" loading="lazy"><figcaption>image-20210723173215728</figcaption></figure>
...
<figure><img src="@source/src/必会工具/assets/image-20210723192605566.png" alt="image-20210723192605566" tabindex="0" loading="lazy"><figcaption>image-20210723192605566</figcaption></figure>
<p>分析：</p>
...
<figure><img src="@source/src/必会工具/assets/image-20210723193730799.png" alt="image-20210723193730799" tabindex="0" loading="lazy"><figcaption>image-20210723193730799</figcaption></figure>
<p>请求<strong>参数与搜索文档的参数完全一致</strong>。</p>
<p>返回值类型就是页面要展示的最终结果：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723203915982.png" alt="image-20210723203915982" tabindex="0" loading="lazy"><figcaption>image-20210723203915982</figcaption></figure>
<p>结果是一个Map结构：</p>
<ul>
...
<p>在<code v-pre>cn.beisheng.hotel.web</code>包的<code v-pre>HotelController</code>中添加一个方法，遵循下面的要求：</p>
<ul>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>    <span class="token annotation punctuation">@PostMapping</span><span class="token punctuation">(</span><span class="token string">"filters"</span><span class="token punctuation">)</span>
    ...
    <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里调用了IHotelService中的getFilters方法，尚未实现。</p>
<p>在<code v-pre>cn.beisheng.hotel.service.IHotelService</code>中定义新方法：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>在<code v-pre>cn.beisheng.hotel.service.impl.HotelService</code>中实现该方法：</p>
...
<figure><img src="@source/src/必会工具/assets/image-20210723204936367.png" alt="image-20210723204936367" tabindex="0" loading="lazy"><figcaption>image-20210723204936367</figcaption></figure>
...
<p>要实现根据字母做补全，就必须对文档按照拼音分词。在GitHub上恰好有elasticsearch的拼音分词插件。地址：<a href="https://github.com/medcl/elasticsearch-analysis-pinyin" target="_blank" rel="noopener noreferrer">https://github.com/medcl/elasticsearch-analysis-pinyin<ExternalLinkIcon/></a></p>
<figure><img src="@source/src/必会工具/assets/image-20210723205932746.png" alt="image-20210723205932746" tabindex="0" loading="lazy"><figcaption>image-20210723205932746</figcaption></figure>
<p>资料中也提供了拼音分词器的安装包：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723205722303.png" alt="image-20210723205722303" tabindex="0" loading="lazy"><figcaption>image-20210723205722303</figcaption></figure>
...
<figure><img src="@source/src/必会工具/assets/image-20210723210126506.png" alt="image-20210723210126506" tabindex="0" loading="lazy"><figcaption>image-20210723210126506</figcaption></figure>
...
<figure><img src="@source/src/必会工具/assets/image-20210723210427878.png" alt="image-20210723210427878" tabindex="0" loading="lazy"><figcaption>image-20210723210427878</figcaption></figure>
<p>声明自定义分词器的语法如下：</p>
...
<figure><img src="@source/src/必会工具/assets/image-20210723211829150.png" alt="image-20210723211829150" tabindex="0" loading="lazy"><figcaption>image-20210723211829150</figcaption></figure>
<p>总结：</p>
<p>如何使用拼音分词器？</p>
<ul>
<li>
...
<p>elasticsearch提供了<a href="https://www.elastic.co/guide/en/elasticsearch/reference/7.6/search-suggesters.html" target="_blank" rel="noopener noreferrer">Completion Suggester<ExternalLinkIcon/></a>查询来实现自动补全功能。这个查询会匹配以用户输入内容开头的词条并返回。为了提高补全查询的效率，对于文档中字段的类型有一些约束：</p>
<ul>
<li>
...
<span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_2-4-2-修改hoteldoc实体" tabindex="-1"><a class="header-anchor" href="#_2-4-2-修改hoteldoc实体" aria-hidden="true">#</a> 2.4.2.修改HotelDoc实体</h3>
<p>HotelDoc中要添加一个字段，用来做自动补全，内容可以是酒店品牌、城市、商圈等信息。按照自动补全字段的要求，最好是这些字段的数组。</p>
<p>因此我们在HotelDoc中添加一个suggestion字段，类型为<code v-pre>List&lt;String&gt;</code>，然后将brand、city、business等信息放到里面。</p>
<p>代码如下：</p>
...
<figure><img src="@source/src/必会工具/assets/image-20210723213546183.png" alt="image-20210723213546183" tabindex="0" loading="lazy"><figcaption>image-20210723213546183</figcaption></figure>
<h3 id="_2-4-4-自动补全查询的javaapi" tabindex="-1"><a class="header-anchor" href="#_2-4-4-自动补全查询的javaapi" aria-hidden="true">#</a> 2.4.4.自动补全查询的JavaAPI</h3>
<p>之前我们学习了自动补全查询的DSL，而没有学习对应的JavaAPI，这里给出一个示例：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723213759922.png" alt="image-20210723213759922" tabindex="0" loading="lazy"><figcaption>image-20210723213759922</figcaption></figure>
<p>而自动补全的结果也比较特殊，解析的代码如下：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723213917524.png" alt="image-20210723213917524" tabindex="0" loading="lazy"><figcaption>image-20210723213917524</figcaption></figure>
...
<figure><img src="@source/src/必会工具/assets/image-20210723214021062.png" alt="image-20210723214021062" tabindex="0" loading="lazy"><figcaption>image-20210723214021062</figcaption></figure>
<p>返回值是补全词条的集合，类型为<code v-pre>List&lt;String&gt;</code></p>
<p>1）在<code v-pre>cn.beisheng.hotel.web</code>包下的<code v-pre>HotelController</code>中添加新接口，接收新的请求：</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>2）在<code v-pre>cn.beisheng.hotel.service</code>包下的<code v-pre>IhotelService</code>中添加方法：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>3）在<code v-pre>cn.beisheng.hotel.service.impl.HotelService</code>中实现该方法：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@Override</span>
...
<figure><img src="@source/src/必会工具/assets/image-20210723214758392.png" alt="image-20210723214758392" tabindex="0" loading="lazy"><figcaption>image-20210723214758392</figcaption></figure>
...
<p>方案一：同步调用</p>
<figure><img src="@source/src/必会工具/assets/image-20210723214931869.png" alt="image-20210723214931869" tabindex="0" loading="lazy"><figcaption>image-20210723214931869</figcaption></figure>
<p>基本步骤如下：</p>
<ul>
...
<p>方案二：异步通知</p>
<figure><img src="@source/src/必会工具/assets/image-20210723215140735.png" alt="image-20210723215140735" tabindex="0" loading="lazy"><figcaption>image-20210723215140735</figcaption></figure>
<p>流程如下：</p>
<ul>
<li>hotel-admin对mysql数据库数据完成增、删、改后，发送MQ消息</li>
<li>hotel-demo监听MQ，接收到消息后完成elasticsearch数据修改</li>
</ul>
<h3 id="_3-1-3-监听binlog" tabindex="-1"><a class="header-anchor" href="#_3-1-3-监听binlog" aria-hidden="true">#</a> 3.1.3.监听binlog</h3>
<p>方案三：监听binlog</p>
<figure><img src="@source/src/必会工具/assets/image-20210723215518541.png" alt="image-20210723215518541" tabindex="0" loading="lazy"><figcaption>image-20210723215518541</figcaption></figure>
<p>流程如下：</p>
<ul>
...
<p>利用资料提供的hotel-admin项目作为酒店管理的微服务。当酒店数据发生增、删、改时，要求对elasticsearch中数据也要完成相同操作。</p>
<p>步骤：</p>
<ul>
<li>
<p>导入资料提供的hotel-admin项目，启动并测试酒店数据的CRUD</p>
</li>
<li>
...
<h3 id="_3-2-2-导入demo" tabindex="-1"><a class="header-anchor" href="#_3-2-2-导入demo" aria-hidden="true">#</a> 3.2.2.导入demo</h3>
<p>导入资料提供的hotel-admin项目：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723220237930.png" alt="image-20210723220237930" tabindex="0" loading="lazy"><figcaption>image-20210723220237930</figcaption></figure>
<p>运行后，访问 <a href="http://localhost:8099" target="_blank" rel="noopener noreferrer">http://localhost:8099<ExternalLinkIcon/></a></p>
<figure><img src="@source/src/必会工具/assets/image-20210723220354464.png" alt="image-20210723220354464" tabindex="0" loading="lazy"><figcaption>image-20210723220354464</figcaption></figure>
<p>其中包含了酒店的CRUD功能：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723220511090.png" alt="image-20210723220511090" tabindex="0" loading="lazy"><figcaption>image-20210723220511090</figcaption></figure>
<h3 id="_3-2-3-声明交换机、队列" tabindex="-1"><a class="header-anchor" href="#_3-2-3-声明交换机、队列" aria-hidden="true">#</a> 3.2.3.声明交换机、队列</h3>
<p>MQ结构如图：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723215850307.png" alt="image-20210723215850307" tabindex="0" loading="lazy"><figcaption>image-20210723215850307</figcaption></figure>
...
<p>在hotel-admin和hotel-demo中的<code v-pre>cn.beisheng.hotel.constatnts</code>包下新建一个类<code v-pre>MqConstants</code>：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">package</span> <span class="token namespace">cn<span class="token punctuation">.</span>beisheng<span class="token punctuation">.</span>hotel<span class="token punctuation">.</span>constatnts</span><span class="token punctuation">;</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">package</span> <span class="token namespace">cn<span class="token punctuation">.</span>beisheng<span class="token punctuation">.</span>hotel<span class="token punctuation">.</span>config</span><span class="token punctuation">;</span>

<span class="token keyword">import</span> <span class="token import"><span class="token namespace">cn<span class="token punctuation">.</span>beisheng<span class="token punctuation">.</span>hotel<span class="token punctuation">.</span>constants<span class="token punctuation">.</span></span><span class="token class-name">MqConstants</span></span><span class="token punctuation">;</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">MqConfig</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_3-2-4-发送mq消息" tabindex="-1"><a class="header-anchor" href="#_3-2-4-发送mq消息" aria-hidden="true">#</a> 3.2.4.发送MQ消息</h3>
<p>在hotel-admin中的增、删、改业务中分别发送MQ消息：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723221843816.png" alt="image-20210723221843816" tabindex="0" loading="lazy"><figcaption>image-20210723221843816</figcaption></figure>
<h3 id="_3-2-5-接收mq消息" tabindex="-1"><a class="header-anchor" href="#_3-2-5-接收mq消息" aria-hidden="true">#</a> 3.2.5.接收MQ消息</h3>
<p>hotel-demo接收到MQ消息要做的事情包括：</p>
<ul>
<li>新增消息：根据传递的hotel的id查询hotel信息，然后新增一条数据到索引库</li>
<li>删除消息：根据传递的hotel的id删除索引库中的一条数据</li>
</ul>
<p>1）首先在hotel-demo的<code v-pre>cn.beisheng.hotel.service</code>包下的<code v-pre>IHotelService</code>中新增新增、删除业务</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>2）给hotel-demo中的<code v-pre>cn.beisheng.hotel.service.impl</code>包下的HotelService中实现业务：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@Override</span>
...
<p>在hotel-demo中的<code v-pre>cn.beisheng.hotel.mq</code>包新增一个类：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">package</span> <span class="token namespace">cn<span class="token punctuation">.</span>beisheng<span class="token punctuation">.</span>hotel<span class="token punctuation">.</span>mq</span><span class="token punctuation">;</span>

<span class="token keyword">import</span> <span class="token import"><span class="token namespace">cn<span class="token punctuation">.</span>beisheng<span class="token punctuation">.</span>hotel<span class="token punctuation">.</span>constants<span class="token punctuation">.</span></span><span class="token class-name">MqConstants</span></span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">cn<span class="token punctuation">.</span>beisheng<span class="token punctuation">.</span>hotel<span class="token punctuation">.</span>service<span class="token punctuation">.</span></span><span class="token class-name">IHotelService</span></span><span class="token punctuation">;</span>
...
<figure><img src="@source/src/必会工具/assets/image-20200104124440086-5602723.png" alt="image-20200104124440086" tabindex="0" loading="lazy"><figcaption>image-20200104124440086</figcaption></figure>
<blockquote>
...
<figure><img src="@source/src/必会工具/assets/image-20200104124551912.png" alt="image-20200104124551912" tabindex="0" loading="lazy"><figcaption>image-20200104124551912</figcaption></figure>
...
<h2 id="_4-1-搭建es集群" tabindex="-1"><a class="header-anchor" href="#_4-1-搭建es集群" aria-hidden="true">#</a> 4.1.搭建ES集群</h2>
<p>参考资料的文档：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723222732427.png" alt="image-20210723222732427" tabindex="0" loading="lazy"><figcaption>image-20210723222732427</figcaption></figure>
<p>其中的第四章节：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723222812619.png" alt="image-20210723222812619" tabindex="0" loading="lazy"><figcaption>image-20210723222812619</figcaption></figure>
...
<figure><img src="@source/src/必会工具/assets/image-20210723223008967.png" alt="image-20210723223008967" tabindex="0" loading="lazy"><figcaption>image-20210723223008967</figcaption></figure>
...
<p>一个典型的es集群职责划分如图：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723223629142.png" alt="image-20210723223629142" tabindex="0" loading="lazy"><figcaption>image-20210723223629142</figcaption></figure>
...
<figure><img src="@source/src/必会工具/assets/image-20210723223804995.png" alt="image-20210723223804995" tabindex="0" loading="lazy"><figcaption>image-20210723223804995</figcaption></figure>
<p>此时，node2和node3认为node1宕机，就会重新选主：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723223845754.png" alt="image-20210723223845754" tabindex="0" loading="lazy"><figcaption>image-20210723223845754</figcaption></figure>
<p>当node3当选后，集群继续对外提供服务，node2和node3自成集群，node1自成集群，两个集群数据不同步，出现数据差异。</p>
<p>当网络恢复后，因为集群中有两个master节点，集群状态的不一致，出现脑裂的情况：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723224000555.png" alt="image-20210723224000555" tabindex="0" loading="lazy"><figcaption>image-20210723224000555</figcaption></figure>
...
<p>插入三条数据：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723225006058.png" alt="image-20210723225006058" tabindex="0" loading="lazy"><figcaption>image-20210723225006058</figcaption></figure>
<figure><img src="@source/src/必会工具/assets/image-20210723225034637.png" alt="image-20210723225034637" tabindex="0" loading="lazy"><figcaption>image-20210723225034637</figcaption></figure>
<figure><img src="@source/src/必会工具/assets/image-20210723225112029.png" alt="image-20210723225112029" tabindex="0" loading="lazy"><figcaption>image-20210723225112029</figcaption></figure>
<p>测试可以看到，三条数据分别在不同分片：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723225227928.png" alt="image-20210723225227928" tabindex="0" loading="lazy"><figcaption>image-20210723225227928</figcaption></figure>
<p>结果：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723225342120.png" alt="image-20210723225342120" tabindex="0" loading="lazy"><figcaption>image-20210723225342120</figcaption></figure>
...
<figure><img src="@source/src/必会工具/assets/image-20210723224354904.png" alt="image-20210723224354904" tabindex="0" loading="lazy"><figcaption>image-20210723224354904</figcaption></figure>
<p><strong>说明：</strong></p>
<ul>
<li><strong>_routing默认是文档的id</strong></li>
<li><strong>算法与分片数量有关，因此索引库一旦创建，分片数量不能修改！</strong></li>
</ul>
<p>新增文档的流程如下：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723225436084.png" alt="image-20210723225436084" tabindex="0" loading="lazy"><figcaption>image-20210723225436084</figcaption></figure>
<p>解读：</p>
<ul>
<li>1）新增一个id=1的文档</li>
...
<figure><img src="@source/src/必会工具/assets/image-20210723225809848.png" alt="image-20210723225809848" tabindex="0" loading="lazy"><figcaption>image-20210723225809848</figcaption></figure>
...
<p>1）例如一个集群结构如图：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723225945963.png" alt="image-20210723225945963" tabindex="0" loading="lazy"><figcaption>image-20210723225945963</figcaption></figure>
<p>现在，node1是主节点，其它两个节点是从节点。</p>
<p>2）突然，node1发生了故障：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723230020574.png" alt="image-20210723230020574" tabindex="0" loading="lazy"><figcaption>image-20210723230020574</figcaption></figure>
<p>宕机后的第一件事，需要重新选主，例如选中了node2：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723230055974.png" alt="image-20210723230055974" tabindex="0" loading="lazy"><figcaption>image-20210723230055974</figcaption></figure>
<p>node2成为主节点后，会检测集群监控状态，发现：shard-1、shard-0没有副本节点。因此需要将node1上的数据迁移到node2、node3：</p>
<figure><img src="@source/src/必会工具/assets/image-20210723230216642.png" alt="image-20210723230216642" tabindex="0" loading="lazy"><figcaption>image-20210723230216642</figcaption></figure>
</div></template>
